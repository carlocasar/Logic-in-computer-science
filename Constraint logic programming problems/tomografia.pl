...
mulMember(L, [], 0, L):-!.
mulMember([X|L], [X|L2], N, L1):- N1 is N-1, mulMember(L, L2, N1, L1). 

matrixByRows([], _, []):-!.
matrixByRows(L, NumCols, [Row|MatrixByRows]):- mulMember(L, Row, NumCols, L1), matrixByRows(L1, NumCols, MatrixByRows).

sumLine([], []).
sumLine([X|L1], [Result|L2]):- sum(X, #=, Result), sumLine(L1, L2).

declareConstraints(MatrixByRows, MatrixByCols, RowSums, ColSums):- 
	sumLine(MatrixByRows, RowSums), sumLine(MatrixByCols, ColSums).
