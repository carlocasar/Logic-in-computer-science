...
eachTaskStartsOnce(Time):- task(T), duration(T,D), findall(start-T-H, (hour(H), TimeTotal is H + D - 1, TimeTotal =< Time), Lits), exactly(1,Lits), fail.
eachTaskStartsOnce(_).

defineHoresOcupat:- task(T), hour(H), duration(T,D), L is H + D - 1 , between(H,L,X), writeClause([\+start-T-H, ocup-T-X]), fail.
defineHoresOcupat.

noSameUnit:- hour(H), resourceUnits(R,U), findall(ocup-T-H,(task(T),usesResource(T,R)),Lits), atMost(U,Lits),fail.
noSameUnit.
